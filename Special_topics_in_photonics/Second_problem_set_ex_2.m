...
%  u1=ifft2(fft2(u0).*exp(-1i*z(m)*(KX.^2+KY.^2)));
 ...
 if m==closestIndex_2
    [w_analytical, w_numerical, data] = calculate_beam_widths(u1, z_, w0, zR, y_, m);
    w0*sqrt(2)
    stop_loops = true;
    [w_analytical_init, w_numerical_init, data_init] = calculate_beam_widths(u1_init, z_, w0, zR, y_, 1);
    figure(5); 
    plot(y_,data.^2); axis square;
%     hold on
%     plot(y_,data_init.^2);
%     hold off
 end
 
figure(3); surf(X,Y,abs(u1).^2); axis([-Lx./2 Lx./2 -Ly./2 Ly./2 0 1]);
...
function [minValue, closestIndex] = findClosestValue(referenceValues, vector)
    % referenceValues: vector of reference values
    % vector: vector to search for closest values
    % minValue: vector of minimum absolute differences between 'vector' and each value in 'referenceValues'
    % closestIndex: vector of indices of the closest values in 'vector' to each value in 'referenceValues'
...
function [w_norm_analytical, w_norm_numerical, data] = calculate_beam_widths(u1, z, w0_norm, zR_norm, y, m)
    %     Analytical solution 
