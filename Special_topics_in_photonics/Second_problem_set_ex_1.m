...
%  max trajectory
u3 = zeros(1, length(u1)); % create vector of zeros
index = find( abs(u1) == max(abs(u1)));
u3(index) = u1(index); field_3(m,:)=u3;
...
figure(3); plot(x,abs(u2),'-b',x,abs(u1),'-r'); axis square;
figure(4); surf(X,Z,abs(u2)); shading interp; colormap(jet); 

axis tight; axis square; view([0 90]); 

% 1b) maximum value trajectory
figure(5); surf(X,Z,abs(field_3)); shading interp; colormap(jet); 
