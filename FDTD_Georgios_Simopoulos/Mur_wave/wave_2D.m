...
kx=linspace(1,IE);
ky = sqrt(2*freq./c0).* sqrt(freq./c0 - kx);
...
% ez (i , 2:1:JE-1 ) = - ez2 (i , 2:1:JE-1 ) + M *( ez (i+1 , 2:1:JE-1 ) + ez2 (i , 2:1:JE-1 ) ) + N1 *( ez1 (i , 2:1:JE-1 ) + ez1 (i+1 , 2:1:JE-1 ) );
    ...
    for j=2:JE-1
        ez(i , j ) = - ez2(i+1 , j ) + M *( ez(i+1 , j ) + ez2(i , j ) ) + N1 *( ez1(i , j ) + ez1(i+1 , j )) + O1 *( ez1(i , j+1) -2* ez1(i , j ) + ez1(i ,j-1) + ez1(i+1 , j+1) -2* ez1(i+1 , j ) + ez1(i+1 ,j-1) ) ;
    end
